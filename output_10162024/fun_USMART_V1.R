...
est = function (data) {
  ...
  IPWE = colMeans(na.omit(cbind(IPWE12, IPWE13, IPWE21, IPWE23, IPWE31, IPWE32)))
  IPWE
...
simul = function(sim1, seed) {
    ...
    if (ar1$s1_YR[i]==1) {ar1$s2_trt[i] = NA}
      ...
      if (length(data) <= 1 || length(unique(data$s2_trt)) <2) {
        ...
        if (ar1$s1_trt[i] == 3) {
          ...
          if (P_s2_good_trt31==0 & P_s2_good_trt32==0) {adp=c(0, 0.5, 0.5)}
  ...
  return(result = list(ev, data=sim1)) 
...
population.data = gen.population.data(seed=3, n=pop_size, ut1=ut1, ut2=ut2)
